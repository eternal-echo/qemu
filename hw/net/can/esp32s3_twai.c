...
{
    Esp32S3TWAIState *s = ESP32S3_TWAI(opaque);
                               ...
                               unsigned int size)
{
    ...
    /* ESP32-S3 hardware only supports PeliCAN mode */
    if (sja_addr == SJA_CDR) {
        value |= 0x80;
    }
...
static void esp32s3_twai_class_init(ObjectClass *klass, void *data)
{
    ...
    esp32s3_class->parent_twai_write = esp32_parent_class->twai_write;
    esp32s3_class->parent_twai_read = esp32_parent_class->twai_read;
    
    esp32_parent_class->twai_write = esp32s3_twai_write;
    esp32_parent_class->twai_read = esp32s3_twai_read;
}
